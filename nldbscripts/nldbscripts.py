...
while text != '':
    ...
    if word[0] is not None:
        ...
        similars1 = search.similars_by_membership(word[0], ms)
        print('Близкие по вхождению: ', [
              str(t[1])+' : '+engine.dbget_word(t[0])+'\n' for t in similars1[0:similars_count]])
        similars2 = search.similars_by_membership(word[0], cs)
        print('Близкие по контексту: ', [
              str(t[1])+' : '+t[0]+'\n' for t in similars2[0:similars_count]])
