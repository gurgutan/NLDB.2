...
namespace NLDB
{
    ...
    //--------------------------------------------------------------------------------------------
    // Здесь описание класса описывающего дополнительный уровень абстракции, который после 
    // размышлений было решено удалить. Закомментированный текст пока оставлен в справочных цедлях.
    //--------------------------------------------------------------------------------------------
    ...
    //    {
    //        string str = ExecuteScalar(db, $"SELECT MAX(cast({column} as INTEGER)) FROM {tablename};").ToString();
